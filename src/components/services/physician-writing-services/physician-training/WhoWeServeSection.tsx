...
export default function WhoWeServeSection() {
  ...
  // Automatically reset mobile state if the window is resized up to desktop view
  useEffect(() => {
    const handleResize = () => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {whoWeServeData.map((card, idx) => {
            ...
            return (
              ...
              >
                {/* Image (Visible by default, vanishes completely on hover/active) */}
                <div
                ...
                <div
                  className={`absolute bottom-0 left-0 right-0 p-5 z-10 flex items-end transition-opacity duration-300 ${isActive ? "opacity-0" : "opacity-100 md:group-hover:opacity-0"
                    }`}
                >
                ...
                {/* Hover/Active Solid Black Overlay with Title + Bullets */}
                <div
                  className={`absolute inset-0 bg-black p-5 sm:p-6 text-white transition-opacity duration-300 z-20 flex flex-col justify-start space-y-3 overflow-y-auto ${isActive
