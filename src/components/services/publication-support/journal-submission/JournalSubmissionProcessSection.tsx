...
const processSteps = [
  ...
  },
  {
    ...
    description: (
      <>
        We format your manuscript according to the target journal’s guidelines and craft a compelling{" "}
        <Link href="/services/publication-support/journal-submission/complete-guide-to-writing-cover-letter-for-medical-journals/">
          cover letter journal submission
        </Link>{" "}
        that highlights the significance of your research and its alignment with the journal’s scope.
      </>
    ),
    iconSrc:
  ...
  return (
    <div className="w-full font-poppins text-gray-800 bg-white">
      ...
      <section className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
          {whoWeServeData.map((item, idx) => (
            <div
              key={idx}
              className="group relative h-56 rounded-sm overflow-hidden shadow-md transition-all duration-300"
            >
              <Image
                src={item.image}
                alt={item.title}
      ...
      <section className="py-6 md:py-8 px-4 md:px-8 bg-[#EAEAEA] w-full text-center font-poppins overflow-hidden">
        ...
        <h3 className="text-lg md:text-xl font-bold text-[#0c3547] mb-2">
          Our step-by-Step Process
        </h3>

        <p className="max-w-4xl mx-auto text-[#4B5563] text-base leading-relaxed mb-2">
...
function StepCard({ step }: { step: (typeof processSteps)[0] }) {
  return (
    <div className="group w-full bg-white rounded-sm p-4 border border-gray-200 shadow-sm flex flex-col items-center text-center transition-all duration-300 hover:bg-[#505050] hover:text-white hover:shadow-2xl hover:-translate-y-1 cursor-pointer min-h-[270px] justify-start">
      {/* Icon with white invert on hover */}
      <div className="w-12 h-12 relative mb-3 transition-all group-hover:brightness-0 group-hover:invert">
        <Image
          src={step.iconSrc}
          alt={step.title}
          fill
      ...
      {/* Card Title */}
      <h3 className="font-bold text-lg text-[#0c3547] group-hover:text-white mb-2 uppercase leading-snug">
        {step.title}
      </h3>

      {/* Card Description */}
      <p className="text-base text-gray-600 group-hover:text-gray-200 leading-relaxed">
        {step.description}
      </p>
    </div>
  );
